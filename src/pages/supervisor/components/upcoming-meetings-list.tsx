...
export function UpcomingMeetingsList({ meetings, isLoading }: UpcomingMeetingsListProps) {
  // Show meetings from 2 hours ago onwards to catch current/recent sessions
  const cutoff = subHours(new Date(), 2);
  
  const upcomingMeetings = meetings
    .filter(m => m.status === 'upcoming' && new Date(m.date_time) >= cutoff)
    ...
    <div className="mb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {upcomingMeetings.map(meeting => (
          <Card key={meeting.id} className="border-none shadow-sm bg-white hover:shadow-md transition-shadow group">
